...
def perceptron_single_step_update(
    ...
    """
    z = label * np.add(np.dot(current_theta, feature_vector), current_theta_0)
    if z <= 0:
        current_theta = np.add(current_theta,  label * feature_vector)
...
def pegasos_single_step_update(
    ...
    Returns:
        ...
        completed.
    """
    if np.multiply(label , (np.dot(theta, feature_vector) + theta_0) )<= 1:
        theta = np.add(np.multiply((1 - eta * L) , theta ), np.multiply(eta * label , feature_vector))
        theta_0 = theta_0 + eta * label
    else:
        theta = np.multiply((1 - eta * L) , theta)
...
def classify(feature_matrix, theta, theta_0):
    ...
    for i in range(len(feature_matrix)):
        prediction = np.add(np.dot(theta, feature_matrix[i]), theta_0)
        if prediction > 0:
